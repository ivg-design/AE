...
const path = require('path');
    ...
    let logContent = '';  // Declare logContent here

// Accept the main file path as a command-line argument
    const mainFilePath = process.argv[2] || path.join(__dirname, 'TuneSync.jsx'); // fallback to a default path
    ...
    const bundledContent = `//========== INCLUDED FUNCTIONS ============//\n${functionContent}//========== END OF INCLUDED FUNCTIONS ============//\n\n${mainContent}`;

    fs.writeFileSync(path.join(basePath, 'dist', 'TuneSync_Bundled.jsx'), bundledContent, 'utf-8');
    fs.writeFileSync(path.join(basePath, 'dist', 'include_log.txt'), logContent, 'utf-8');
