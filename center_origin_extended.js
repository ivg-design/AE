/**
 * Originator: Center Shape Path Origins in Adobe After Effects
 *
 * Provides both headless processing and a dockable UI panel for centering the origin of shape paths on Shape Layers.
 * - Automatically centers a single shape path by adjusting its vertices so that its centroid aligns with the origin.
 * - When multiple shape paths are present, displays a ScriptUI tree view that lets users select the shape path to be centered.
 *
 * FUNCTIONALITY:
 * - Recursively traverses a Shape Layer’s "Contents" to extract vector groups and shape path properties.
 * - Computes the centroid of a shape path and offsets its vertices to recenter the path to (0,0).
 * - Mimics After Effects’ property path construction to reliably access shape path properties.
 * - Provides a user-friendly UI for selection when dealing with complex, nested shape hierarchies.
 *
 * USAGE:
 * 1. Open a composition and select a Shape Layer.
 * 2. Run the script.
 *    - If the layer contains only a single shape path, it is centered immediately.
 *    - If multiple shape paths are detected, a tree view dialog is displayed.
 * 3. In the dialog, select one shape path.
 * 4. Click "Center Selected" to center the origins of the selected shape path.
 *
 * @changelog
 * 1.0.0 - Initial release of OriginAligner.
 * @author IVG Design
 * @name OriginAligner
 * @version 1.0.0
 * @date 2025-02-06
