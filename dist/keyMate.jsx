/**
 * KeyMate Script: Modify Keyframe Values in Adobe After Effects
 *
 * Provides a dockable UI panel that allows users to:
 * - Input values for X, Y, and Z dimensions.
 * - Use checkboxes to enable or disable specific dimensions.
 * - Apply mathematical operations (+, -, ×, ÷, =) to selected keyframe values.
 *
 * FUNCTIONALITY:
 * - Displays a panel with input fields for X, Y, and Z values, checkboxes, and operation buttons.
 * - Allows users to enable or disable dimensions dynamically using checkboxes.
 * - Performs operations (+, -, ×, ÷, =) on the selected keyframes based on the user's input.
 * - Supports 2D and 3D properties (e.g., Position, Scale) and processes only selected keyframes.
 * - Ensures no changes are made to unselected keyframes.
 *
 * USAGE:
 * 1. Open a composition and select a layer with keyframes.
 * 2. Highlight the property with keyframes to be modified.
 * 3. Run the script to display the dockable panel.
 * 4. Input values for X, Y, and/or Z, then check the corresponding dimension checkboxes to enable them.
 * 5. Click an operation button (+, -, ×, ÷, =) to modify the selected keyframes.
 *
 * @changelog
 * 1.0.1 - fixed property value type check
 * @author IVG Design
 * @name Keymate
 * @version 1.0.1
 * @date 2025-01-16
 ...
 */
{
	function addKeyMateScript(thisObj) {
		function buildUI(thisObj) {
			var panel = (thisObj instanceof Panel) ? thisObj : new Window("palette", "🔸 KeyMate 🔸", undefined, { resizeable: true });
				...
				function processKeyframes(operation) {
					...
					for (var i = 0; i < selectedLayers.length; i++) {
						...
						for (var j = 0; j < properties.length; j++) {
							...
							if (property.propertyValueType === 6417 ||
								property.propertyValueType === 6423 ||
								property.propertyValueType === 6424 ||
								property.propertyValueType === 6414 ||
								property.propertyValueType === 6413 ||
								property.propertyValueType === 6416 ||
								property.propertyValueType === 6415) {
		...
		var keyMate = buildUI(thisObj);

		if (keyMate !== null && keyMate instanceof Window) {
			keyMate.center();
			keyMate.show();
		}
	}
	addKeyMateScript(this);
}
