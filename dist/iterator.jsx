...
/*********** Initialize the UI ***********/
	...
	okButton.onClick = function () {
		...
		if (startLayerIndex === endLayerIndex && startPropertyPath === endPropertyPath) {
			alert("Start property and end property cannot be the same. Please select different properties.");
			return;
		}
		...
		if (startProperty && endProperty) {
			...
			var numLayers = maxIndex - minIndex - 1;

			if (numLayers <= 0) {
				alert("Please select more than two layers for this operation.");
				return;
			}
			...
			// Determine the direction of the step based on start and end layer indices
			var direction = (startLayerIndex < endLayerIndex) ? 1 : -1;

			for (var i = 1; i <= numLayers; i++) {
				var curLayerIndex = (direction === 1) ? minIndex + i : maxIndex - i;
				var curProperty = getPropertyFromPath(curComp.layer(curLayerIndex), startPropertyPath);
				...
				if (curProperty && curProperty.canVaryOverTime) {
					...
					if (is_array(stepValue)) {
						...
						for (var j = 0; j < stepValue.length; j++) {
							newValue[j] = startProperty.value[j] + (stepValue[j] * i);
						}
					} else {
						newValue = startProperty.value + (stepValue * i);
					}
